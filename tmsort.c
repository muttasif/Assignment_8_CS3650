...
/**
 * Forward declaration. See description below.
 */
...
/**
 * Helper function which is exectued only as thread context.
 */
...
void merge_sort_aux(long nums[], int from, int to, long target[], int thread_count)
{
  ...
  pthread_t thread;
  merge_ctx args = {target, nums, from, mid, ((thread_count + 1) / 2)};
  /* Run one part in parallel */
  pthread_create(&thread, NULL, merge_sort_aux_threaded, &args);
  /* Run another part in current thread */
